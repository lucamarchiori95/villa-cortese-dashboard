...
# Aggrega partiti storici con nomi cambiati nel tempo.
# 'azione' viene cercato solo a inizio stringa per evitare
# false corrispondenze con 'rifondazione'.
# =========================================================
def normalize_party(n):
    ...
    # PD e coalizioni di centro-sinistra storiche
    if any(x in n for x in ["partito democratico", "ulivo", "uniti nell'ulivo",
                              "l'ulivo", "margherita", "democratici sinistra",
                              "dl.la margherita", "progressista"]):
        return "PD"
    if n == "pd" or n.startswith("pd ") or n.endswith(" pd"):
        return "PD"

    # FI / PDL e coalizioni di centro-destra
    if any(x in n for x in ["forza italia", "pdl", "popolo della liberta",
                              "casa delle liberta", "moderati"]):
        return "FI / PDL"

    # LEGA (tutte le varianti storiche)
    if any(x in n for x in ["lega nord", "lega salvini", "lega per salvini"]):
        return "LEGA"
    if n == "lega" or n.startswith("lega ") or "-lega" in n:
        return "LEGA"

    # FDI / AN (Alleanza Nazionale confluita in FDI)
    if any(x in n for x in ["fratelli d'italia", "fratelli d\u2019italia",
                              "alleanza nazionale"]):
        return "FDI"

    # M5S
    if any(x in n for x in ["movimento 5 stelle", "5 stelle", "m5s", "beppegrillo"]):
        return "M5S"

    # AVS
    if any(x in n for x in ["alleanza verdi", "verdi e sinistra",
                              "avs", "sinistra ecologia"]):
        return "AVS"

    # AZIONE — solo se il nome INIZIA con "azione" (evita "rifondazione")
    if n.startswith("azione"):
        return "AZIONE"
...
# =========================================================
# TREND
# - aggrega partiti con stesso nome normalizzato
# - deduplica righe per stesso anno (media se duplicati)
# - mostra solo partiti con >= 3 tornate E picco >= 3%
# - sull'asse X compaiono SOLO gli anni delle elezioni
# =========================================================
def build_trend(df):
    if df.empty:
        return {}
    ...
    for _, r in df.iterrows():
        ...
        if val is None:
            continue
        tmp.setdefault((p, anno), []).append(val)

    res = {}
    for (p, anno), vals in tmp.items():
        res.setdefault(p, {})
        res[p][anno] = round(sum(vals) / len(vals), 2)

    filtered = {}
    for p, values in res.items():
        if len(values) >= 3 and max(values.values()) >= 3:
            filtered[p] = values
...
def build_snapshot(df):
    ...
    for _, r in df.iterrows():
        ...
        if val is None:
            continue
        tmp.setdefault(p, []).append(val)

    res = {p: round(sum(v) / len(v), 1) for p, v in tmp.items()}
    return sorted(res.items(), key=lambda x: -x[1]), anno_max


# =========================================================
# COMUNALI — identico al codice originale
# =========================================================
...
# =========================================================
# FIGURE TREND
# Asse X: solo anni in cui c'è stata effettivamente l'elezione
# =========================================================
def make_trend_fig(title, data, party, color):
    if not party or party not in data:
        fig = go.Figure()
        fig.update_layout(
            title=f"{title} - dati non disponibili",
...
def make_bar_fig(title, snap, anno):
    ...
    fig.update_layout(
        title=dict(text=f"{title} - {anno}", x=0.5),
        template="plotly_white",
        height=350,
        dragmode=False,
        hovermode=False,
        margin=dict(l=35, r=10, t=50, b=60),
        xaxis=dict(fixedrange=True),
        yaxis=dict(
            fixedrange=True,
            ticksuffix="%",
            gridcolor="#e8e8e8",
            range=[0, max(valori) + 15]
        )
    )
    return fig
...
# =========================================================
# LAYOUT COMUNALI — identico al codice originale
# =========================================================
...
app.layout = dbc.Container([
    ...
    html.Hr(),
    html.P("Dati elaborati automaticamente - uso non ufficiale",
